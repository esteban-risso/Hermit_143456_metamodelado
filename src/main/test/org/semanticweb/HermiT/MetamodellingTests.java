...
public class MetamodellingTests extends TestCase {
	...
	public void testAccountingConsistente2_corta() {
		CommandLine cl = new CommandLine();
		flags.add(testCasesPath+"Nuevos/AccountingConsistente2_corta.owl");
		cl.main(flags.toArray(new String[flagsCount+1]));
		System.out.println("AccountingConsistente2_corta es consistente");
	...
	public void testAccountingConsistente2_media() {
		CommandLine cl = new CommandLine();
		flags.add(testCasesPath+"Nuevos/AccountingConsistente2_media.owl");
		cl.main(flags.toArray(new String[flagsCount+1]));
		System.out.println("AccountingConsistente2_media es consistente");
		
		flags.remove(flagsCount);
		TestCase.assertEquals(true, true);
	}
	
	public void testMetadosConceptos() {		
		CommandLine cl = new CommandLine();
		flags.add(testCasesPath+"Nuevos/prueba_ind_meta_dos_conceptos.owl");
		boolean result = false;
		try {
			cl.main(flags.toArray(new String[flagsCount+1]));
		}catch (InconsistentOntologyException e) {
			System.out.println("testMetadosConceptos es inconsistente");
			result = true;
		}
		flags.remove(flagsCount);
		TestCase.assertEquals(true, result);
	...
	public void testTestCaseH4corto() {
		CommandLine cl = new CommandLine();
		flags.add(testCasesPath+"Nuevos/TestCaseH4corto.owl");
		boolean result = false;
		try {
			cl.main(flags.toArray(new String[flagsCount+1]));
		}catch (InconsistentOntologyException e) {
			System.out.println("TestCaseH4corto es inconsistente");
			result = true;
		}
